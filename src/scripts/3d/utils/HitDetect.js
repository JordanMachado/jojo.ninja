...
class HitDetect extends EventEmitter
{
    ...
    _checkHit()
    {
        ...
        for (let i = 0; i < this.objects.length; i++)
        {
            ...
            {
                object.geometry.faces = generateFaces(object.geometry);
            }
            ...
            for (let j = 0; j < faces.length; j++)
            {
                ...
                getVector(v0, vertices[0], object.transform3d.matrixWorld);
                getVector(v1, vertices[1], object.transform3d.matrixWorld);
                getVector(v2, vertices[2], object.transform3d.matrixWorld);
...
function generateFaces(mGeometry)
{
    const bufferIndex = mGeometry.attributes.position.buffer;
    const positions = mGeometry.buffers[bufferIndex].data;
    const indices = mGeometry.buffers[mGeometry.buffers.length - 1].data;
